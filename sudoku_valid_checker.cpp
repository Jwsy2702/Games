...
void extract_row(int r, const std::vector<int>& in, std::vector<int>& out){
    int side = std::sqrt(in.size());
      for(int i = 0; i < side; i++){
    out.push_back(in[xytoi(r, i, side)]);
    }
}
...
void extract_col(int c, const std::vector<int>& in, std::vector<int>& out){
    int side = std::sqrt(in.size());
    for (int i = 0; i < side; i++){
      out.push_back(in[xytoi(i, c, side)]);
    }
}
...
void extract_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    int side = std::sqrt(in.size());
    int subsqside = std::sqrt(side);
    int rowoffset = (subs/subsqside);
    int columnoffset = (subs%subsqside);

    for (int i = rowoffset*subsqside; i < (rowoffset + 1)*subsqside; i++){
        for (int j = columnoffset*subsqside; j < (columnoffset + 1)*subsqside; j++){
        out.push_back(in[xytoi(i,j,side)]);
        }
    }
